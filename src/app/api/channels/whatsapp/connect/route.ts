...
const APP_SECRET = metaServerEnv.whatsappAppSecret;
const APP_ID = metaServerEnv.whatsappAppId;
  ...
  try {
    ...
    if (!APP_ID || !APP_SECRET) {
      return NextResponse.json(
        {
          error:
            "NEXT_PUBLIC_META_APP_ID/NEXT_PUBLIC_WHATSAPP_APP_ID dan META_APP_SECRET/WHATSAPP_APP_SECRET belum dikonfigurasi di environment variables Vercel.",
        },
        { status: 500 },
      );
    }
    ...
    if (!workingToken) {
      return NextResponse.json({ error: "accessToken atau code diperlukan." }, { status: 400 });
    }
    ...
    const phoneUrl = new URL(`${META_GRAPH}/${API_VERSION}/${wabaId}/phone_numbers`);

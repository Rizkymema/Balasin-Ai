...
const WhatsAppIcon = () => (
  <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-emerald-50 border border-emerald-200 shadow-2xs shrink-0">
    <svg className="h-3.5 w-3.5 fill-emerald-600 text-emerald-600" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
const InstagramIcon = () => (
  <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-pink-50 border border-pink-200 shadow-2xs shrink-0">
    <svg className="h-3.5 w-3.5 text-pink-600 fill-none stroke-[2px]" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
const WebsiteChatIcon = () => (
  <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-blue-50 border border-blue-200 shadow-2xs shrink-0">
    <svg className="h-3.5 w-3.5 text-blue-600 fill-none stroke-[2px]" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
export function ContactsTablePanel({
    ...
    switch (channel) {
      case "WhatsApp":
        return (
          <div className="flex items-center gap-2">
            <WhatsAppIcon />
            <span className="text-emerald-700 text-xs font-bold">WhatsApp</span>
          </div>
        );
      case "Instagram DM":
      case "Instagram Comment":
        return (
          <div className="flex items-center gap-2">
            <InstagramIcon />
            <span className="text-pink-700 text-xs font-bold">Instagram</span>
          </div>
        );
      default:
        return (
          <div className="flex items-center gap-2">
            <WebsiteChatIcon />
            <span className="text-blue-700 text-xs font-bold">{channel}</span>
          </div>
        );
    }
  };
  ...
  return (
    <Card className="relative space-y-5 p-5 md:p-6 bg-white border border-slate-200 rounded-2xl shadow-2xs">
      ...
      <div className="grid gap-3 md:grid-cols-[minmax(0,1.5fr)_repeat(2,minmax(0,1fr))]">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9 h-10 bg-slate-50 border-slate-200 hover:border-slate-300 focus:border-blue-600 transition rounded-xl text-xs"
      ...
      {paginatedRows.length === 0 ? (
        ...
        <>
                ...
                <TableRow>
                  ...
                  <TableHead className="w-[50px] px-4 py-3">
                    <input
                      ...
                      onChange={toggleAll}
                      className="h-4 w-4 rounded border-slate-300 bg-white text-blue-600 focus:ring-blue-500 cursor-pointer"
                    />
                  </TableHead>
                  <TableHead className="px-4 py-3 font-bold text-xs">Name</TableHead>
                  <TableHead className="px-4 py-3 font-bold text-xs">Channels</TableHead>
                  <TableHead className="px-4 py-3 font-bold text-xs">Last updated</TableHead>
                  <TableHead className="px-4 py-3 text-right font-bold text-xs">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {paginatedRows.map((row) => (
                  <TableRow
                    ...
                    className={cn(
                      ...
                      row.id === selectedId ? "bg-blue-50/40" : undefined,
                    )}
                  >
                    {/* Checkbox Single */}
                    ...
                    <TableCell className="px-4 py-3">
                      <button
                        ...
                        className="flex items-center gap-3 text-left hover:opacity-80 transition group cursor-pointer"
                      >
                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-blue-200 bg-blue-50 text-xs font-bold text-blue-700 shadow-2xs">
                          {row.customer.name.slice(0, 1).toUpperCase()}
                        </span>
                        <div className="min-w-0">
                          <span className="block truncate text-xs font-bold text-slate-900 group-hover:text-blue-600 transition">
                            {row.customer.name}
                          </span>
                    ...
                    <TableCell className="px-4 py-3 text-right">
                      <Dropdown
                        trigger={
                          <button className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition cursor-pointer">
          ...
          <div className="grid gap-3 lg:hidden">
            {paginatedRows.map((row) => (
              <div
                key={row.id}
                className={cn(
                  "rounded-2xl border p-4 text-left transition-all duration-200 relative shadow-2xs",
                  row.id === selectedId
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      checked={selectedRowIds.has(row.id)}
                      onChange={() => toggleOne(row.id)}
                      className="h-4 w-4 rounded border-slate-300 bg-white text-blue-600 cursor-pointer"
                    />
                    <button
                      type="button"
                      onClick={() => onSelect(row.id)}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-900">{row.customer.name}</span>
          ...
          {selectedRowIds.size > 0 && (
            <div className="flex items-center justify-between gap-4 px-4 py-3 bg-slate-900 border border-slate-800 rounded-xl shadow-lg animate-slide-in-up">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  variant="secondary"
                  className="h-8 text-xs font-bold px-3 py-1 bg-white/10 text-white border-white/10 hover:bg-white/20 rounded-lg shadow-2xs"
                >
                ...
                <Button
                  ...
                  className="h-8 text-xs font-bold px-3 py-1 bg-rose-600 text-white hover:bg-rose-700 rounded-lg border-transparent shadow-2xs"
                >
                ...
                <button
                  ...
                  className="text-xs text-slate-400 hover:text-white px-2 py-1 transition cursor-pointer font-bold"
                >
                  Batal
                </button>
              </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pt-4 border-t border-slate-100">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500 font-semibold">
              ...
              <select
                ...
                className="bg-white border border-slate-200 rounded-lg px-2 py-1 text-slate-800 font-bold cursor-pointer focus:border-blue-600 outline-none shadow-2xs"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <span className="text-[11px] text-slate-400 font-bold text-center sm:text-right">
              ...
              <div className="flex items-center justify-center gap-1">
                <button
                  ...
                  className="p-1.5 rounded-lg border border-slate-200 bg-white text-slate-400 hover:text-slate-700 transition disabled:opacity-40 disabled:pointer-events-none cursor-pointer shadow-2xs"
                >
                ...
                {Array.from({ length: totalPages }).map((_, index) => {
                  ...
                  if (!shouldShow) {
                    if (pageNum === 2 || pageNum === totalPages - 1) {
                      return <span key={pageNum} className="px-1 text-slate-300 text-xs font-bold">...</span>;
                    }
                    return null;
                  }
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "h-8 w-8 rounded-lg text-xs font-bold transition cursor-pointer shadow-2xs",
                        currentPage === pageNum

...
export function ContactDetailPanel({
  ...
  return (
    <Card className="space-y-5 p-5 md:p-6">
      <section className="space-y-4 rounded-2xl border border-[var(--color-border)] bg-white/[0.03] p-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="flex items-start gap-4">
            <span className="flex h-14 w-14 items-center justify-center rounded-full border border-cyan-400/20 bg-cyan-950/30 text-lg font-bold text-cyan-200">
            ...
            <div className="space-y-3">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-xl font-bold text-white">{detail.customer.name}</h2>
          ...
          <div className="flex flex-wrap gap-2 xl:max-w-[380px] xl:justify-end">
            ...
            <Button type="button" variant="secondary" className="h-10 rounded-xl px-4" onClick={onCreateDeal}>
              <WalletCards className="mr-2 h-4 w-4" />
              Create Deal
            </Button>
            <Button type="button" variant="secondary" className="h-10 rounded-xl px-4" onClick={onAddTask}>
            ...
            <Button
              type="button"
              variant="secondary"
              className="h-10 rounded-xl border-rose-400/20 bg-rose-950/20 px-4 text-rose-200 hover:bg-rose-950/30"
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
        <div className="space-y-5">
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
            <Card className="space-y-4 p-4">
              <div>
                <h3 className="text-sm font-semibold text-white">Informasi Customer</h3>
            ...
            <Card className="space-y-4 p-4">
              <div>
                <h3 className="text-sm font-semibold text-white">Identity Resolution</h3>
                <p className="mt-1 text-xs leading-6 text-slate-400">
          ...
          <Card className="space-y-4 p-4">
            <div>
              <h3 className="text-sm font-semibold text-white">Tags & Context</h3>
              <p className="mt-1 text-xs leading-6 text-slate-400">
          ...
          <Card className="space-y-3 p-4">
            <div>
              <h3 className="text-sm font-semibold text-white">Consent & Readiness</h3>
              <p className="mt-1 text-xs leading-6 text-slate-400">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
                ...
                <p className="mt-2 text-sm font-semibold text-white">Manual follow-up</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-5">
          <Card className="space-y-4 p-4">
            <div>
              <h3 className="text-sm font-semibold text-white">Timeline Aktivitas</h3>
              <p className="mt-1 text-xs leading-6 text-slate-400">
          ...
          <Card className="space-y-4 p-4">
            <div>
              <h3 className="text-sm font-semibold text-white">Related Data</h3>
              <p className="mt-1 text-xs leading-6 text-slate-400">
                ...
                <div className="mt-3 space-y-2">
                  {detail.relatedDeals.length > 0 ? (
                    detail.relatedDeals.slice(0, 3).map((deal) => (
                      <div key={deal.id} className="rounded-xl border border-white/8 bg-white/[0.03] p-3">
                ...
                <div className="mt-3 space-y-2">
                  {detail.relatedBookings.length > 0 ? (
                    detail.relatedBookings.slice(0, 3).map((booking) => (
                      <div key={booking.id} className="rounded-xl border border-white/8 bg-white/[0.03] p-3">
                ...
                <div className="mt-3 space-y-2">
                  {detail.relatedTickets.length > 0 ? (
                    detail.relatedTickets.slice(0, 3).map((ticket) => (
                      <div key={ticket.id} className="rounded-xl border border-white/8 bg-white/[0.03] p-3">
